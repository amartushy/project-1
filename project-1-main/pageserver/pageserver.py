...
def serve(sock, func, options):
    ...
    while True:
        ...
        _thread.start_new_thread(func, (clientsocket, options))
...
def respond(sock, options):
    ...
    if len(parts) > 1 and parts[0] == "GET":
        filepath = parts[1][1:]  # Remove initial slash
        if ".." in filepath or "~" in filepath:
            transmit(STATUS_FORBIDDEN, sock)
            transmit("\nForbidden request: {}\n".format(request), sock)
            return
        full_path = os.path.join(options.DOCROOT, filepath)
